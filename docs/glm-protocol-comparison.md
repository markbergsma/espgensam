...
2. ~~**Swap the `0x43` / `0x45` labels.**~~ **Done.** `0x43` = HF/tweeter, `0x45` = LF/woofer, confirmed again on live hardware: with music playing, a 7350A reported `0x43` at the `0x80` floor in 164/164 frames while `0x45` swung `0x80`–`0xF1`. Behaviour is unchanged (`max()` across channels). `0x46` was relabelled at the same time — it is subwoofer-only, but has been seen both dominating while `0x45` floored and floored while `0x45` swung, so calling it "the subwoofer's output" overstates what is known.
...
7. Optional feature work, in rough order of value: ISS sleep delay / sensitivity / LED via `3A 01/02/06`; startup and max level via `10 01`; time-of-flight delay via `10 02`; PEQ via `10 0E`. Anything persisted needs `15 33 00`. **PEQ, level and delay done** — see §7.1; the group preset feature transmits `17 01`, `10 02`, `10 01 00`, `10 0E` ×20, `3B` and `0x40` per speaker. Level (`10 01 00`) and delay (`10 02`) are also reachable per speaker outside a group, as Home Assistant number entities, sent without a preceding `17 01`: the captures show GLM omitting it for individual writes. The snoop decoder follows `10 01 00` and `10 02` back, so a change made in GLM is picked up rather than reverted at the next standby cycle; `10 01 09` is deliberately not decoded, since its constant `000000` payload would read as -130 dB and mute the speaker. Startup level (`10 01 05`), PEQ snooping, ISS settings and flash commit are not implemented.
8. ~~**Support `0x3B 00 01` as full band (§4.5).**~~ **Done.** Implemented on the mode-enum reading: `CROSSOVER_FULL_BAND = 0x0001` in `const.h`, anything else is Hz on the 50–120 / 5 Hz grid. The supporting evidence is now two independent sources rather than one: GLM's own setup file stores `CrossoverFrequency(Hz):1` and `Group_Crossover:1` for a sub-less group, alongside 80/85/90 for groups with a subwoofer. Group YAML spells it `crossover: full_band`, `sam_file:` / `sam2yaml.py` map GLM's `1` to it, and the per-monitor Home Assistant control became a select (`Full band`, `50 Hz` … `120 Hz`) because a number entity could only clamp `1` to 50 Hz. An AutoPhase angle in a full-band group yields no delay, since there is no crossover to realise it at. Still untested on the wire from espgensam; the §8 experiment stands.
